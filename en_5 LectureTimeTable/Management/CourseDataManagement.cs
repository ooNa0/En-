...
namespace en_5_LectureTimeTable
{
    class CourseDataManagement
    {
        ...
        public void AddCourse(List<CourseVO> courseList, List<CourseVO> editCourses, List<CourseVO> referenceCourses, int addInterestedCourse) // 강의 추가
        {
            ...
            if (searchOrBack == "1")
            {
                SearchCourse(courseList, referenceCourses, addInterestedCourse);
            }
            else if(searchOrBack == "b") { return; }
            Console.WriteLine("(뒤로가기 \"b\" 하고 ENTER)");
            ...
            for (i = 0; i < courseList.Count; i++)
            {
                if (courseList[i].No == Convert.ToDouble(searchOrBack))
                {
                    ...
                    {
                        if(editCourses[j].CourseTitle == courseList[i].CourseTitle) // 같은 과목?
                        {
                            count++;
                        }
                        if (editCourses[j].CourseNumber == courseList[i].CourseNumber) // 같은 학수 번호?
                        {
                            count++;
                        }
                    ...
                    {
                        Console.WriteLine("과목, 학수번호, 시간이 겹치지 않는지 확인해주세요.");
                        print.NoticeBack();
                        return;
                    }
                }
        ...
        public void SearchCourse(List<CourseVO> courseList, List<CourseVO> interestedCourseList, int addInterestedCourse) // 강의 찾기, addInterestedCourse는 수강신청시에 1, 아닐시에 0이다.
        {
        ...
        public void InputSearch(List<CourseVO> courseList, string menuNumber)
        {
            int count = 0;
            int search = 0;
            ...
            switch (menuNumber)
            {
                ...
                case Constant.BY_COURSE_NUMBER: // 학수 번호
                    ...
                    while (count < courseList.Count)
                    {
                        if (courseList[count].CourseNumber.Contains(inputedString))
                        {
