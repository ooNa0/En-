...
namespace en_6_Library_DB
{
    class AdministratorManagement
    {
            ...
            while (!isFinished)
            {
                ...
                switch (inputManagement.GetMenuNumber(Constant.MAXIMUN_ADMINISTRATOR_MENU_NUMBER))
                {
                    ...
                    case Constant.PRINT_USERLIST:
                        outputData.ShowAllUserList(dataAccessObject.GetAllBookData(Constant.USER_TABLE));
                    ...
                    case Constant.REMOVE_USER: // 유저 삭제
                        outputData.ShowAllUserList(dataAccessObject.GetAllBookData(Constant.USER_TABLE));
                        input = inputManagement.GetIdentity(false);
            ...
            while (!isFinished)
            {
                ...
                switch (inputManagement.GetMenuNumber(Constant.MAXIMUN_ADMINISTRATOR_BOOKMANAGEMENT_MENU_NUMBER))
                {
                    ...
                    case Constant.NAVER_API_REGISTRATION_BOOK: // 네이버 책 등록
                        naverAPIAccessObject.ShowNaverBookList(inputManagement.GetBookTitle(), Constant.IS_REGISTRATION, inputManagement.GetNaverBookDisplay());
                        break;
                    ...
                    case Constant.EDIT_BOOK:
                        ...
                        if(bookNOString == Constant.BACK) { continue; }
                        bookNumberString = inputManagement.GetBookNumber();
                        if(bookNumberString == Constant.BACK) { continue; }
        ...
        public void RegistrateBook() // 책 등록, 아이디 겹치는지 확인 예외처리 필요
        {
            ...
            dataAccessObject.InputBookDateInDataBase(title, author, publisher, bookNumber, price, null, null, null);
            Console.WriteLine("{0} 도서 등록이 완료되었습니다! ENTER를 눌러주세요.\n", title);
