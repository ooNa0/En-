...
namespace en_6_Library_DB
{
    class Menu
    {
        ...
        public Menu()
        {
            ...
            inputManagement = new InputManagement(exception, outputError, dataAccessObject);
            bookManagement = new BookManagement(outputMenu, outputData, inputManagement, dataAccessObject, log);
            userManagement = new UserManagement(exception, inputManagement, outputMenu, dataAccessObject, outputError, outputData, bookManagement, log);
            ...
            while (!hasEnded) {
                ...
                switch (inputManagement.GetMenuNumber(Constant.MAXIMUN_FIRST_MENU_NUMBER))
                {
                    ...
                    case Constant.ADMINISTRATOR_MODE:
                        Console.Write("별하나 입니다. * 이거! 관리자 비밀번호를 입력해주세요 :");
