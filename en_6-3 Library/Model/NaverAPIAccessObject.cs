...
namespace en_6_Library_DB
{
    class NaverAPIAccessObject
    {
        ...
        public void ShowNaverBookList(string query, bool isRegistration, string display)
        {
            string stringLog;
            int index = 0;
            string discount;
            
            string url = "https://openapi.naver.com/v1/search/book.json?query=" + query + Constant.NAVER_API_DISPLAY + display; // 결과가 JSON 포맷
            ...
            if (isRegistration)
            {
                ...
                // 책 등록
                Console.WriteLine("도서 등록 완료, ENTER를 눌러주세요.");
